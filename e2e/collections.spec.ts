...
for (const collection of collections) {
	test.describe(`${collection.path} listing`, () => {
		test("loads with a heading and at least one card", async ({ page }) => {
			...
			await expect(
				page.getByRole("heading", { name: collection.heading }),
			).toBeVisible();
		...
		test("search filters the list", async ({ page }) => {
			...
			const initialCount = await page
				.locator(`a[href^="${collection.path}/"]`)
				.count();
			expect(initialCount).toBeGreaterThan(0);

			await search.fill("zzz-no-such-content-zzz");
			await expect(page.getByText("Nothing found.")).toBeVisible();
			await expect(page.locator(`a[href^="${collection.path}/"]`)).toHaveCount(
				0,
			);
		...
		test("clicking a card opens the detail page and back link returns", async ({
			page,
		}) => {
			await page.goto(collection.path);
