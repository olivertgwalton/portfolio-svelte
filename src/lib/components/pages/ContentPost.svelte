...
<article class="bg-surface-50-950 pb-32">
	<header class="border-b border-surface-200-800/80 pt-32 pb-16">
		<div class="container mx-auto max-w-7xl px-6">
			...
			>
				<span aria-hidden="true"><ArrowLeftIcon size={16} /></span>
				BACK TO {isProject ? "PROJECTS" : "BLOGS"}
			</a>
				...
				<div class="max-w-3xl">
					...
					>
						<div class="flex items-center gap-2">
							...
							<span class="font-bold">{formatDate(meta.date)}</span>
						</div>
						{#if meta.readTime}
							<div class="flex items-center gap-2">
			...
			<section class="mt-16 border-t border-surface-200-800 pt-12">
				...
				<div class="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
					{#each related as item (item.slug)}
						<ContentCard {item} collection={type} variant="compact" />
					{/each}
				</div>
			</section>
		{/if}
