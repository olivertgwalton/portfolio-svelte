...
<figure class={wrapperClass}>
	...
	>
		...
		<Portal>
			<Dialog.Backdrop class="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm" />
			<Dialog.Positioner class="fixed inset-0 z-50 flex items-center justify-center p-4">
				<Dialog.Content class="relative flex max-h-full max-w-full items-center justify-center">
					<Dialog.Title class="sr-only">{alt || 'Expanded image'}</Dialog.Title>
					<Dialog.Description class="sr-only">Expanded image preview</Dialog.Description>
					<Dialog.CloseTrigger
