...
import { socialLinks } from "#lib/site.config.ts";
...
<ul
	class="{isLarge ? 'space-y-4 font-heading text-2xl font-bold text-surface-950-50' : 'space-y-4 text-lg font-medium text-surface-800-200'}"
>
		...
		<li use:reveal={{ delay: animated ? 450 + i * 50 : 0 }}>
			...
			>
				<span aria-hidden="true"
					><Icon
						size={isLarge ? 24 : 18}
						weight={isLarge ? 'bold' : 'regular'}
						class="shrink-0 text-surface-600-400"
					/></span
				>
				{label}
				<span aria-hidden="true"
